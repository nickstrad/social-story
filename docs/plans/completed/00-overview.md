...
- **OpenAI** REST API (chat completions with `json_schema` response format; `images/generations` and `images/edits`). **API key env var is `OPENAI_TOKEN`**. Default models: chat `gpt-5.5`, image `gpt-image-2`, env-overridable via `OPENAI_CHAT_MODEL` / `OPENAI_IMAGE_MODEL`.
