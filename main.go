...
//   3. redo     -> regenerate one page referenced from the JSON file as a new
//                  variant (page2_1, page2_2 ...), with the same photo step.
//
...
//	social-story redo     story.json  <page#> [outDir]   # regenerate one page
package main
...
func buildImagePrompt(scene string, hasPeople, anchored bool, photoNote string) string {
...
func main() {
	...
	case "redo":
		args := rawArgs
		if len(args) < 2 {
			fatal("usage: social-story redo story.json <page#> [outDir] [--caption]")
		}
		story := loadStory(args[0])
		pageNum, err := strconv.Atoi(args[1])
		if err != nil {
			fatal("page# must be an integer: %v", err)
		}
		outDir := optArg(args, 2, "images")
		mustRedo(client, story, pageNum, outDir, basePath, caption)

	case "caption":
		// Deterministically add the text band to already-generated page images.
		args := rawArgs
		if len(args) < 1 {
			fatal("usage: social-story caption story.json [outDir]")
		}
		story := loadStory(args[0])
		outDir := optArg(args, 1, "images")
		mustCaption(story, outDir)
...
func mustCaption(story Story, outDir string) {
	n := 0
	for _, p := range story.Pages {
		out := filepath.Join(outDir, fmt.Sprintf("page%d.png", p.Page))
...
func renderOne(c *openAIClient, p Page, out string, lib photoLibrary, anchor string, caption bool, pr *progress) error {
	...
	prompt := buildImagePrompt(p.ImagePrompt, p.hasPeople(), anchored, photoNote)
...
func mustRedo(c *openAIClient, story Story, pageNum int, outDir, basePath string, caption bool) {
	var page *Page
	...
	next := nextVariant(outDir, pageNum)
	out := filepath.Join(outDir, fmt.Sprintf("page%d_%d.png", pageNum, next))

	pr := newProgress()
	pr.startHeartbeat()
	pr.begin(pageNum, 1, "starting")
	err := renderOne(c, *page, out, loadPhotoLibrary(), anchorPath(basePath), caption, pr)
	pr.finish(pageNum, err)
	pr.stopHeartbeat()
	if err != nil {
		fatal("%v", err)
	}
...
Commands:
  ...
  redo     story.json <page#> [outDir]  regenerate one page as a new variant (page2_1, page2_2...)
...
Typical order: parse -> base -> generate --page 1 (test) -> generate (all)
